...
// React Native Firebase auto-initializes from google-services.json, but we ensure it's ready here
(async () => {
  try {
    // Wait a tiny bit for native Firebase initialization from google-services.json
    // React Native Firebase initializes automatically, but we need to wait for it
    let attempts = 0;
    ...
    try {
      const apps = firebase.apps;
      if (apps && apps.length === 0) {
        console.warn('⚠️ [index.js] Firebase not initialized after waiting - may cause issues');
      }
    } catch (error) {
      console.warn('⚠️ [index.js] Firebase initialization check failed:', error?.message);
    }
  } catch (error) {
    console.error('❌ [index.js] Error ensuring Firebase initialization:', error);
